...
TEMP_DIR = os.path.join(PROJECT_ROOT, "temp_work")
...
def run_forgery_pipeline(task_id: str, uploaded_blueprint_path: str):
    ...
    # Define paths for intermediate files within a temporary directory
    temp_blueprint_path = os.path.join(TEMP_DIR, f"{task_id}_blueprint.json")
    training_data_path = os.path.join(TEMP_DIR, f"{task_id}_training_data.csv")
    model_h_path = os.path.join(TEMP_DIR, f"{task_id}_model.h")
    ...
    try:
        ...
        print(f"[{task_id}] Running Oracle's Cradle...")

        model_process = subprocess.run(
            [
                "python",
                os.path.join("AI", "ML", "train_oracle.py"),
                "--data-file",
                training_data_path,
                "--output-header",
                model_h_path,
        ...
        print(f"[{task_id}] Model Trainer Output:\n{model_process.stdout}")
        if not os.path.exists(model_h_path):
            raise FileNotFoundError(
                "Model training did not produce the expected model.h file."
            )

        # STEP 3: Update Blueprint with new model path
        tasks_db[task_id][
            "message"
        ] = "Step 3/4: Integrating AI model into blueprint..."
        ...
        blueprint_data["ai_model"] = {
            ...
            "output_classes": ["Normal", "Leak", "Burst", "Clog", "Drift"],
        }
        ...
        print(f"[{task_id}] Running IntelliPipe Factory...")

        project_name = blueprint_data.get("project_details", {}).get("name", "output")
        final_artifact_name = f"{project_name}_FACTORY_BUILT.ino"
        final_artifact_path = os.path.join(
            ARTIFACTS_DIR, f"{task_id}_{final_artifact_name}"
        )
        ...
        if not os.path.exists(final_artifact_path):
            raise FileNotFoundError(
                "Firmware forging did not produce the expected .ino file."
            )
        ...
        error_message = (
            f"Pipeline failed at: {tasks_db[task_id]['message']}. Error: {e.stderr}"
        )
        tasks_db[task_id]["status"] = "failed"
        tasks_db[task_id]["message"] = error_message
...
async def forge_firmware(
    background_tasks: BackgroundTasks, blueprint: UploadFile = File(...)
):
    if blueprint.filename is None or not blueprint.filename.endswith(".json"):
        raise HTTPException(
            status_code=400,
            detail="Invalid file type. Please upload a .json blueprint.",
        )
...
async def download_artifact(task_id: str):
    ...
    if task["status"] != "completed" or not task["artifact_path"]:
        raise HTTPException(
            status_code=404, detail="Artifact not ready or task failed."
        )

    artifact_path = task["artifact_path"]
    if not os.path.exists(artifact_path):
        raise HTTPException(
            status_code=404, detail="Artifact file not found on server."
        )
...
async def root():
    return {
        "message": "IntelliPipe Orchestration Server is online. POST your blueprint to /forge to begin."
    }
