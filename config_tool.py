...
USER_CPP_TEMPLATE = r"""// IntelliPipe Master C++ Template v12.4 "Aegis"
#include <FS.h>
#include <SPIFFS.h>
#include <WiFi.h>
...
def build_system_graph(bp):
        ...
        for p in pipes:
            ...
            if 'flow_sensor' in p and p['flow_sensor']: # ensure it's not an empty dict
                ...
                if not s_id or s_id in s_map: 
                    raise ValueError(f"Blueprint is sensored but contains invalid/duplicate sensor ID in pipe {p.get('pipe_id')}")
    ...
    for idx, p in enumerate(pipes):
        if p.get('source_node') in io: io[p['source_node']]['out'].append(idx)
        if p.get('target_node') in io: io[p['target_node']]['in'].append(idx)
        ...
        for p in pipes:
            if 'flow_sensor' in p and p['flow_sensor']:
                ...
                if s_idx is not None:
                    if p.get('source_node') in b_io: b_io[p['source_node']]['out'].append(s_idx)
                    if p.get('target_node') in b_io: b_io[p['target_node']]['in'].append(s_idx)
        
        balance_nodes = [{'id': nid, 'inflows': sorted(d['in']), 'outflows': sorted(d['out'])} for nid, d in b_io.items() if d['in'] or d['out']]
    ...
    print(f" -> Topology analysis complete. Discovered {len(nodes_map)} total nodes and {len(pipes)} total pipes.")
    if is_sensored_blueprint:
        print(f" -> Identified {len(sensors_ordered)} sensors and {len(balance_nodes)} balance nodes for firmware.")
...
def generate_isrs(graph):
    ...
    num_sensors = len(graph['sensors_ordered'])
    return "\n".join([f"void IRAM_ATTR ISR_S{i}() {{ if ({i} < {num_sensors}) volatile_pulse_counts[{i}]++; }}" for i, _ in enumerate(graph['sensors_ordered'])])
...
def generate_wifi_helpers(graph, blueprint):
    ...
    cpp_code = [
        ...
        "            server.send(400, \"text/plain\", \"Bad Request: SSID or password is too long.\");",
        "            return;",
        "        }",
...
def run_master_orchestrator(blueprint, template):
    """
    (Grand Assembler v12.7 - "Aegis Citadel") Final hardened version with
    TLS, optimized median filter, and ISR-safe fixed-size arrays.
    """
    ...
    key_pem, cert_pem = generate_self_signed_cert(blueprint.get('project_details', {}).get('name', 'IntelliPipe'))
    ...
    # --- ISR Safety Hardening (Task 16.C) ---
    ...
    master_code.extend([
        "WebServerSecure server(server_cert, server_key);",
    ...
    def _format_cpp_value(v):
        """A robust helper to correctly format Python values for C++."""
    ...
    log_event_body=["if(ActiveTuning.max_log_events<=0)return;","String logEntry=String(millis()/1000)+\"s: \"+eventMessage;","eventLog.push_back(logEntry);","while(eventLog.size()>(unsigned int)ActiveTuning.max_log_events){eventLog.pop_front();}"]
    set_tuning_body=["for(const auto&p:AllProfiles){if(p.id==profileId){ActiveTuning=p;logEvent(\"Tuning: \"+p.label);return;}}","logEvent(\"WARN: Tuning profile not found: \"+profileId);"]
    calculate_node_stats_body=["if(calibrationSamplesCollected<2)return false;","double mean_d=node.sum_X/calibrationSamplesCollected;","double var_d=(node.sum_X2/calibrationSamplesCollected)-(mean_d*mean_d);","if(var_d<0.0)var_d=0.0;","node.mean_balance_lps=(float)mean_d;","node.std_dev_balance_lps=(float)sqrt(var_d);","return true;"]
    
    # --- Median Filter Optimization (Task 16.C) ---
    ...
    median_filter_body=[
        ...
        "if (s.history.empty()) return 0.0f;",
        "std::vector<float> temp_hist = s.history;",
        "size_t median_idx = temp_hist.size() / 2;",
        "std::nth_element(temp_hist.begin(), temp_hist.begin() + median_idx, temp_hist.end());",
        "return temp_hist[median_idx];"
    ]
    # --- End Optimization ---
    ...
    if ai_enabled:
        ...
        all_funcs.extend([generate_full_cpp_function("setup_tflm",["/* AI DISABLED */"]), generate_full_cpp_function("runInferenceAndInterpret",["/* AI DISABLED */"])])
...
def generate_self_signed_cert(project_name):
    ...
    cert.get_subject().O = "IntelliPipe Factory"
...
###def convert_svg_to_blueprint(svg_filepath):
    ...
    try:
        ...
        print("  -> ❌ CONVERSION FAILED: The 'lxml' library is required. Please run 'pip install lxml'")
        return None
    ...
    try:
        ...
        print(f"  -> ❌ CONVERSION FAILED: Could not parse the SVG file. Error: {e}")
        return None
    ...
    for circle in root.findall('.//svg:circle', namespaces=ns):
        ...
        if not node_id:
            print("  -> ⚠️ WARNING: Found a <circle> element without an 'id'. It will be ignored.")
            continue
        nodes.append({
            "node_id": node_id,
    ...
    for line in root.findall('.//svg:line', namespaces=ns):
        pipe_id = line.get('id')
        if not pipe_id:
            print("  -> ⚠️ WARNING: Found a <line> element without an 'id'. It will be ignored.")
            continue
        try:
            ...
            print(f"  -> ⚠️ WARNING: Pipe '{pipe_id}' has invalid or missing coordinates. It will be ignored.")
            continue

        pipes.append({
            "pipe_id": pipe_id,
            "x1": x1, "y1": y1, "x2": x2, "y2": y2, # Store coords temporarily for connection logic
            "properties": {
    ...
    if not nodes or not pipes:
        print("  -> ❌ CONVERSION FAILED: No valid <circle> nodes and <line> pipes with 'id' attributes were found.")
        return None
    ...
    node_coords = {
        node['node_id']: (
            float(tree.find(f'.//svg:circle[@id="{node["node_id"]}"]', namespaces=ns).get('cx')),
            float(tree.find(f'.//svg:circle[@id="{node["node_id"]}"]', namespaces=ns).get('cy'))
        )
        for node in nodes
    }
    ...
    for pipe in pipes:
        ...
        p1_distances = {np.sqrt((pipe['x1'] - nc[0])**2 + (pipe['y1'] - nc[1])**2): nid for nid, nc in node_coords.items()}
        pipe['source_node'] = p1_distances[min(p1_distances.keys())]

        # Find the closest node to the end of the pipe
        p2_distances = {np.sqrt((pipe['x2'] - nc[0])**2 + (pipe['y2'] - nc[1])**2): nid for nid, nc in node_coords.items()}
    ...
    print(f"  -> ✅ CONVERSION SUCCESSFUL. Blueprint saved to '{output_filename}'.")
