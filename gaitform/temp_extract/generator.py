...
class CADGenerator:
    ...
    def generate_mesh(self, params: OrthoticParams) -> trimesh.Trimesh:
        """
        ...
        x_min_pts = np.array([-0.2, -0.25, -0.35, -0.15, -0.48, -0.35, -0.2]) * self.width
        ...
        medial_mask = xx_flat < 0  # assuming left foot, medial is negative X
        arch_mask = midfoot_mask & medial_mask
        
        # simple Gaussian bump for the arch
        arch_center_y = self.length * 0.4
        arch_center_x = -self.width / 2
        dist_to_arch = np.sqrt((xx_flat - arch_center_x)**2 + (yy_flat - arch_center_y)**2)
        arch_bump = np.exp(-(dist_to_arch**2) / (2 * 20**2)) * params.arch_height_mm
        zz_top[arch_mask] += arch_bump[arch_mask]
        
        # 3. Metatarsal pad
        met_mask = (yy_flat > params.met_pad_position_mm - 20) & (yy_flat < params.met_pad_position_mm + 20)
        dist_to_met = np.sqrt((xx_flat)**2 + (yy_flat - params.met_pad_position_mm)**2)
    ...
    def generate_zoning(self, params: OrthoticParams, output_json: str, output_png: str):
        """
        ...
        # Simplified zone rectangles for plotting
        plt.fill_between([-self.width/2, self.width/2], 0, self.length*0.25, color='red', alpha=0.5, label='Heel')
        plt.fill_between([-self.width/2, self.width/2], self.length*0.25, self.length*0.65, color='orange', alpha=0.5, label='Midfoot')
        plt.fill_between([-self.width/2, self.width/2], self.length*0.65, self.length, color='green', alpha=0.5, label='Forefoot')
        
        plt.xlim(-self.width/2 - 10, self.width/2 + 10)
        plt.ylim(-10, self.length + 10)
        plt.title("Rigidity Zones Heatmap")
        plt.xlabel("Width (mm)")
