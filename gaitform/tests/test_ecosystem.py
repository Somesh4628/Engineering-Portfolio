...
def test_cad_generator_right_foot_arch_mirror():
    ...
    midfoot_left = left_verts[
        (left_verts[:, 1] > gen.length * 0.3) & (left_verts[:, 1] < gen.length * 0.5)
    ]
    medial_max_z_l = (
        np.max(midfoot_left[midfoot_left[:, 0] > 0][:, 2])
        if any(midfoot_left[:, 0] > 0)
        else 0
    )
    lateral_max_z_l = (
        np.max(midfoot_left[midfoot_left[:, 0] < 0][:, 2])
    ...
    assert (
        medial_max_z_l > lateral_max_z_l + 2.0
    ), "Left foot arch should be medial (+X)"

    # Sample right foot: Medial is -X, Lateral is +X
    right_verts = mesh_right.vertices
    midfoot_right = right_verts[
        (right_verts[:, 1] > gen.length * 0.3) & (right_verts[:, 1] < gen.length * 0.5)
    ]
    medial_max_z_r = (
        np.max(midfoot_right[midfoot_right[:, 0] < 0][:, 2])
        if any(midfoot_right[:, 0] < 0)
        else 0
    )
    lateral_max_z_r = (
        np.max(midfoot_right[midfoot_right[:, 0] > 0][:, 2])
    ...
    assert (
        medial_max_z_r > lateral_max_z_r + 2.0
    ), "Right foot arch should be medial (-X)"
