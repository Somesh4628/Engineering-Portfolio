...
def test_cad_generation():
    ...
    for params in params_list:
        ...
        # Watertightness is not strictly guaranteed post-subdivision before slicing/Solidworks
...
def test_right_foot_mirror_bug():
    ...
    arch_region_l = mesh_l.vertices[
        (mesh_l.vertices[:, 1] > 0.2 * generator.length)
        & (mesh_l.vertices[:, 1] < 0.5 * generator.length)
    ]
    ...
    arch_region_r = mesh_r.vertices[
        (mesh_r.vertices[:, 1] > 0.2 * generator.length)
        & (mesh_r.vertices[:, 1] < 0.5 * generator.length)
    ]
    ...
    assert max_z_right_negative > max_z_right_positive
